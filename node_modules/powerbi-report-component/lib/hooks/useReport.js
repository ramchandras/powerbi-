...
function _useReport(performOnEmbed) {
    var _a = react_1.useState(null), report = _a[0], _setEmbedInstance = _a[1];
    var setEmbed = function (embedDivRef, embedConfig) {
        var errors = config_1.validateConfig(embedConfig);
        if (!errors || errors.length === 0) {
            embed(embedDivRef.current, embedConfig);
        }
        else {
            var errorString = config_1.parseConfigErrors(errors);
...
function useReport() {
    var _a = react_1.useState(null), report = _a[0], _setEmbedInstance = _a[1];
    var embed = function (ref, config) {
        var embedConfig = config_1.createEmbedConfigBasedOnEmbedType(config);
        var errors = config_1.validateConfig(embedConfig);
        if (!errors || errors.length === 0) {
...
function useBootstrap() {
    var _a = react_1.useState(false), isBotstrapped = _a[0], setIsBootstrapped = _a[1];
    var _b = react_1.useState(null), report = _b[0], _setEmbedInstance = _b[1];
    var embed = function (ref, config) {
        if (isBotstrapped) {
            var embedConfig = config_1.createEmbedConfigBasedOnEmbedType(config);
            var errors = config_1.validateConfig(embedConfig);
    ...
    var bootstrap = function (ref, config) {
        var bootstrapConfig = config_1.createEmbedConfigBasedOnEmbedType(config);
        if (config_1.validateBootrapConfig(bootstrapConfig)) {
